...
4. **Config override layer (#99, #102, #110, #119, #121–#125)** — `config_overrides.json` allows runtime parameter changes without editing `config.py`. Credential keys blocked. Type coercion, zero/negative guard, non-dict protection, OverflowError guard, math.isfinite() guard for float constants (NaN/±Infinity rejection), and full project-tree path containment (replaces specific-subdir blocklist — covers `.env`, source dirs, and home directory root).
5. **Operational reliability fixes** — low-disk guard before recording and in watchdog split path (#107, #127); send_photo resize + backoff + thumbnail-path fix (#106, #113); Telegram API error redaction in event log (#101); SIGTERM shutdown deadline increased to 300 s to outlast legitimate work (#108, #111, #112, #126); SIGTERM race fixed (#112).
6. **Test hardening (#94, #95, #102, #107, #110, #115, #117, #121–#127)** — session-scoped conftest isolation; regression tests for day/night fix, override layer, disk guard, background-pixel gate, Stage A filter, path containment, NaN/Infinity coercion, shutdown deadline value, and watchdog disk-full path.
...
**Tests:** 121 passing, 2 xfailed (known limitations #114, #120). Covers `config`, `storage`,
...
#19, #60, #93, #94, #95, #96, #97, #98, #99, #100, #101, #102, #103, #104, #105, #106, #107, #108, #109, #110, #111, #112, #113, #115, #116, #117, #118, #119, #121, #122, #123, #124, #125, #126, #127
...
├── 03-tests/                # pytest unit tests (121 passing, 2 xfailed)
