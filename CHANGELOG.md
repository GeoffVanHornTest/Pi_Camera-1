...
- **Stage A instant-step gate disabled after any fully-dark frame (#117)** — the Stage A guard `prev_gate_brightness > 0.0` treated a fully-dark frame (legitimate at night when headlights are absent) as an uninitialised state. Any dark frame wrote `0.0` to `_last_gate_brightness`, and the `> 0.0` guard then skipped Stage A permanently for the rest of the session. Fixed to `!= 0.0` — the actual sentinel for "no previous brightness reading" which only applies at module initialisation.
