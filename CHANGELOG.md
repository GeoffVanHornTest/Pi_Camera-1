...
- **Scene-wide false triggers from lighting transitions (#96, #19)** — MOG2 cannot distinguish a global illumination change from real motion. Confirmed root cause from 2026-07-22 field data (10 false positives, 07:48–08:28): camera AGC/AEC stepped discretely during sunrise, raising mean frame brightness by ~10+ gray units, which MOG2 classified as near-full-frame foreground (frame 0 of clip 07:48:09: single contour, 919 601 px² on a 921 600 px² frame). A scene-change gate (`_is_scene_transition()`) now tracks a 150-frame (~5 s) rolling brightness window and suppresses detection for `SCENE_CHANGE_SUPPRESS_SEC = 10 s` when the end-to-end delta exceeds `SCENE_CHANGE_THRESHOLD = 5.0` gray units. MOG2 continues updating during suppression so the background model re-adapts. A `SCENE_CHANGE` entry is appended to the event log when the gate fires. Threshold calibration: 5.0 catches AGC steps (~10+ units) while ignoring sunrise drift (~0.03 units per 5 s) and sensor noise (~1–2 units peak-to-peak).
